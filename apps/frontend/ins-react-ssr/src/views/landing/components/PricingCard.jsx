...
const PricingCard = ({
  ...
  return <Card className={`h-100 bg-light bg-opacity-10 rounded-3 ${plan.isPopular ? 'border-dashed border' : 'border-light'}`}>
      <CardBody className="px-lg-4 p-5 pb-2 text-center">
        ...
        <ul className="list-unstyled text-start fs-sm fw-medium mb-0">
          {plan.features.map((feature, index) => <li key={index} className="mb-2">
      ...
      {plan.isPopular && <span className="position-absolute top-0 start-50 translate-middle-x badge bg-primary-subtle text-primary rounded-pill px-3 py-1 mt-3">
          Best Value
        </span>}
    </Card>;
};
