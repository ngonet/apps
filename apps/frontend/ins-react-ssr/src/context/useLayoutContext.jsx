...
const LayoutProvider = ({
  ...
  const [settings, setSettings] = useLocalStorage('__INSPINIA_NEXT_CONFIG__', INIT_STATE);
  ...
  const changeOrientation = useCallback((nOrientation, persist = true) => {
  ...
  const changeLayoutPosition = useCallback((nPosition, persist = true) => {
    toggleAttribute('data-layout-position', nPosition);
    if (persist) updateSettings({
      position: nPosition
    });
  }, [updateSettings]);
  const changeLayoutWidth = useCallback((nWidth, persist = true) => {
  ...
  return <LayoutContext.Provider value={useMemo(() => ({
    ...
    showBackdrop,
    hideBackdrop
  }), [settings, changeSkin, changeTheme, changeOrientation, changeTopBarColor, changeSideNavSize, changeSideNavColor, changeLayoutPosition, changeLayoutWidth, customizer])}>
      {children}
